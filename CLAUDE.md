...
- `transactions.js` also holds a Svelte store, `transactionInProgress`, used to accumulate state across the multi-step "record an expense" flow (`/expense/who` → `/expense/category` → `/expense/account` → `/expense/amount` → `/expense/review`) before a single `savePendingTransaction()` call persists it and updates category balances.
